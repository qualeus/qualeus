#include "../../include/Overlay/ImGuiHelper.hpp"
...
void imgui_init(GLFWwindow* window, bgfx::ProgramHandle program) {
    ...
    g_imgui_font_texture = bgfx::createTexture2D((uint16_t)width, (uint16_t)height, false, 1, bgfx::TextureFormat::BGRA8, 0, bgfx::copy(data, width * height * 4));
...
void imgui_events(float dt) {
    ...
    if (!(io.ConfigFlags & ImGuiConfigFlags_NoMouseCursorChange) && glfwGetInputMode(g_imgui_window, GLFW_CURSOR) != GLFW_CURSOR_DISABLED) {
        ...
        if (imgui_cursor == ImGuiMouseCursor_None || io.MouseDrawCursor) {
            ...
            glfwSetCursor(g_imgui_window, g_imgui_cursors[imgui_cursor] ? g_imgui_cursors[imgui_cursor] : g_imgui_cursors[ImGuiMouseCursor_Arrow]);
...
void imgui_render() {
    ...
    for (int ii = 0, num = draw_data->CmdListsCount; ii < num; ++ii) {
        ...
        if (!bgfx::getAvailTransientVertexBuffer(num_vertices, g_imgui_vertex_layout) || !bgfx::getAvailTransientIndexBuffer(num_indices)) { break; }
        ...
        for (const ImDrawCmd *cmd = draw_list->CmdBuffer.begin(), *cmdEnd = draw_list->CmdBuffer.end(); cmd != cmdEnd; ++cmd) {
            if (cmd->UserCallback) {
                ...
                if (cmd->TextureId != NULL) { th.idx = uint16_t(uintptr_t(cmd->TextureId)); }
